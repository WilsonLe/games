...
function RestaurantGame({ onExit }: { onExit: () => void }) {
  ...
  const revealGuestOrder = useCallback((guest: ActiveGuest) => {
    ...
    setFeedback({
      ...
      text: isGuidedFirstOrder
        ...
        : hasAudio
          ? `${guest.customer.name}: ${guest.phrase}`
  ...
  }, [activeGuests, gameStatus, guidedGuestId, introOrderComplete, now, playSound, resetCombo]);
  ...
  const dishWishSnapshot: DishWishSnapshot = useMemo(
    ...
    [activeGuests, beltFoods, difficulty.beltTravelMs, gameStatus, guidedGuest, guidedGuestId, introOrderComplete, introStep, introTargetFoodId, now, selectedGuestId],
  );
