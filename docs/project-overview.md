...
Amsoft Games is a client-only React/Vite game portal. The current playable game is `Conveyor Kitchen`.

`Conveyor Kitchen` is a 2D food-serving arcade game for practicing practical English food orders. Guests ask for meals in English, food appears on a two-lane conveyor, and the player clicks food to complete guest orders before their patience timers expire.
...
| Food belt | Ordered foods and decoy foods move right-to-left across two lanes. |
...
| `src/App.tsx` | Game data, components, gameplay state, timers, order generation, scoring, TTS, and sound effects. |
| `src/styles.css` | Full visual system: palette, layout, game scene layers, cards, conveyor, sprites, animation, responsive rules, cursor. |
...
The repo currently supports one game, one page, generated/static visual assets, speech/audio in browser APIs, and local static builds. It does not include user accounts, saved progress, analytics, server APIs, routes, automated tests, or deployment configuration.
