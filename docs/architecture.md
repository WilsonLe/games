...
- Recap recovery uses a spawn-delay invariant rather than patience compensation: the guest-spawn
  ...
  replacement can be made. Expiration itself is resolved against any same-sample blocked-supply
  compensation that the ordered-food effect will grant, so a due requested dish cannot be retried with
  `SUPPLY_DELAY_RETRY_MS` compensation and still expire from the pre-compensation snapshot.
  `nextGuestAtRef` may remain overdue, but after the recap clears only one replacement is created and
  the ref is rebased from that actual spawn time. The replacement's `serviceStartsAt`, `expiresAt`,
  and scheduled-food ready/due times are therefore all derived after the recap; no recap duration is
