...
| `TARGET_SERVES`, `MAX_MISSES`, `ORDERS_PER_LEVEL`, `ORDER_LANES` | Main balancing constants. |
...
| `App` | All live state, refs, effects, controls, scoring, and render tree. |
...
`Conveyor Kitchen` is a single-screen 2D arcade game. Guests ask for English food orders, food rides across a two-lane conveyor, and the player clicks the right food to satisfy active guest orders before patience timers expire.

The current app is a portal shell with one active game. There is no route system, backend, persistence, multiplayer, automated tests, or CI configuration in the repo.
